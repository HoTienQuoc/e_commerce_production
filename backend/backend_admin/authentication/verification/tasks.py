...
def send_verification_email_task(self, user_id:int):
    ...
    try:
        ...
        if not user.is_verified: # pyright: ignore[reportAttributeAccessIssue]
            logger.info(f"Executing verification email task for user: {user.email}") # pyright: ignore[reportAttributeAccessIssue]
            EmailService.send_verification_email(user)
        else:
            logger.info(f"Skipping verification email for already verified user: {user.email}") # pyright: ignore[reportAttributeAccessIssue]
