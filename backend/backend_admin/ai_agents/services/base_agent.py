...
class BaseAgent(ABC):
    ...
    def __init__(self, agent_model: 'Agent'):
        ...
        self.model = getattr(settings, 'DEEPSEEK_MODEL', 'deepseek/deepseek-r1-0528:free')
    ...
    def call_mcp_tool(self, tool_name: str, arguments: Dict[str, Any])->Dict[str, Any]:
        ...
        logger.info(f"Agent '{self.agent_model.name}' calling MCP tool '{tool_name}'...")
        ...
        try:
            ...
            if 'error' in response:
                ...
                return {
                    "error": f"Tool call failed: {error_info.get('message')}"
                }

            tool_content_str = response.get('result', {}).get('content', [{}])[0].get('text', '{}')
            return self.safe_json_parse(tool_content_str)

        except Exception as e:
            logger.error(f"Critical error calling MCP tool '{tool_name}' : {str(e)}", exc_info=True)
            return {"error": f"Failed to communicate with MCP Server: {str(e)}"}

    def start_execution(self, input_data: Optional[Dict[str, Any]] = None)-> 'AgentExecution':
        """Start tracking agent execution"""
        self.excution_record = AgentExecution.objects.create(
            agent = self.agent_model, status = 'running', 
            input_data = input_data or {}, 
            started_at = timezone.now()
        )
        return self.excution_record

    def complete_execution(self, output_data: Optional[Dict[str, Any]] = None, execution_time: Optional[float] = None):
    ...
    def execute_llm_task(self, prompt: str, system_prompt: Optional[str] = None, temperature: float = 0.7, max_tokens: int = 2000)->Optional[str]:
    ...
    def create_recommendation(self, rec_type: str, title: str, description: str, priority: str = 'medium', data:Optional[dict] = None, related_object = None, confidence: float = 0.0, estimated_impact: Optional[Dict] = None)->'AgentRecommendation':
    ...
    def safe_json_parse(self, json_string: str, fallback: Optional[Dict[str, Any]] = None)->Dict:
