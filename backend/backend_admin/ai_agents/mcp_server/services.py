...
class MCPService:
    """Single service to manage the MCP server instance and its tools"""
    _instance = None
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.server = MCPServer( # pyright: ignore[reportAttributeAccessIssue]
                name="E-commerce MCP Server",
                version="1.0"
            )

            cls._instance.server.initialized = True # pyright: ignore[reportAttributeAccessIssue]

            # Register all available tools
            cls._instance._register_tools() 

            logger.info(f"MCP Service initialized with {len(cls._instance.server.tools)}") # pyright: ignore[reportAttributeAccessIssue]
        return cls._instance

    def _register_tools(self):
        """Register all e-commerce tools with the server instance"""
        # Sales Analytics Tool
        self.server.register_tool( # pyright: ignore[reportAttributeAccessIssue]
    ...
    def get_server(self)->MCPServer:
        ...
        return self.server # pyright: ignore[reportAttributeAccessIssue]
