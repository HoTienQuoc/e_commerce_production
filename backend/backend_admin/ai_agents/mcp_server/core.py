...
from collections.abc import Callable  # Thêm dòng này để định nghĩa kiểu dữ liệu
...
class MCPServer:
    ...
    def __init__(self, name:str, version:str):
        ...
        self.tool_handlers: Dict[str, callable] = {} # pyright: ignore[reportGeneralTypeIssues]
        self.resource_handlers: Dict[str, callable] = {} # pyright: ignore[reportGeneralTypeIssues]
        self.prompt_handlers: Dict[str, callable] = {} # pyright: ignore[reportGeneralTypeIssues]
        self.initialized = False

    def register_tool(self, tool: MCPTool, handler: callable): # pyright: ignore[reportGeneralTypeIssues]
        """Register a tool with its handler"""
        self.tools[tool.name] = tool
        self.tool_handlers[tool.name] = handler

    def register_resources(self, resource: MCPResource, handler: callable): # pyright: ignore[reportGeneralTypeIssues]
        """Register a resource with its handler"""
        self.resources[resource.uri] = resource
        self.resource_handlers[resource.uri] = handler

    def register_prompt(self, prompt: MCPPrompt, handler: callable): # pyright: ignore[reportGeneralTypeIssues]
        """Register a prompt with its handler"""
        self.prompts[prompt.name] = prompt
        self.prompt_handlers[prompt.name] = handler

    async def handle_message(self, message: Dict[str, Any])->Dict[str, Any]: # pyright: ignore[reportReturnType]
    ...
    async def _handle_list_tools(self, msg: MCPMessage)->Dict[str, Any]:
        ...
        tool_name = msg.params.get("name") # pyright: ignore[reportOptionalMemberAccess]
        arguments = msg.params.get("arguments", {}) # pyright: ignore[reportOptionalMemberAccess]
        if tool_name not in self.tool_handlers:
            return self._create_error_response(msg.id, -32602, f"Tool '{tool_name}' not found")
        try:
            ...
            return self._create_error_response(msg.id, -32603, f"Tool execution failed: {str(e)}")
    ...
    async def _handle_read_resource(self, msg: MCPMessage)->Dict[str, Any]:
        ...
        if not self.initialized:
            return self._create_error_response(msg.id, -32002, "Server not initialized")
        uri = msg.params.get("uri") # pyright: ignore[reportOptionalMemberAccess]
        if uri not in self.resource_handlers:
            return self._create_error_response(msg.id, -32602, f"Resource '{uri}' not found")
        try:
            handler = self.resource_handlers(uri) # pyright: ignore[reportCallIssue]
            ...
            return self._create_error_response(msg.id, -32603, f"Resource read failed: {str(e)}")
    ...
    async def _handle_get_prompt(self, msg: MCPMessage)->Dict[str, Any]:
        ...
        if not self.initialized:
            return self._create_error_response(msg.id, -32002, "Server not initialized")
        prompt_name = msg.params.get("name") # pyright: ignore[reportOptionalMemberAccess]
        arguments = msg.params.get("arguments", {}) # pyright: ignore[reportOptionalMemberAccess]
        if prompt_name not in self.prompt_handlers:
            return self._create_error_response(msg.id, -32602, f"Prompt '{prompt_name}' not found")
        try:
            handler = self.prompt_handlers(prompt_name) # pyright: ignore[reportCallIssue]
            result = await handler(arguments)
            return MCPMessage(
                id=msg.id,
                result={
            ...
            return self._create_error_response(msg.id, -32603, f"Prompt execution failed: {str(e)}")

    def _create_error_response(self, msg_id: Optional[Union[str, int]], code: int, message: str)->Dict[str, Any]:
