...
class _AddCategoryDialogState extends State<AddCategoryDialog>
  ...
  void _validateName() {
    ...
    BlocProvider.of<CategoryBloc>(
      context,
    ).add(ValidateCategoryNameEvent(name: _nameController.text));
  }
  ...
  Future<void> _pickImage() async {
    try {
      ...
      if (result.isNotEmpty) {
        final bytes = await result.first.xFile.readAsBytes();
        ...
        final mimeType =
            'image/${result.first.extension?.toLowerCase() ?? 'jpeg'}';
          ...
          Stack(
            children: [
              Container(
                ...
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: Image.memory(_imageFile!, fit: BoxFit.cover),
                ),
              ),
              Positioned(
                top: 5,
  ...
  Widget _buildActiveSwitch() {
    return Row(
      children: [
        ...
        Expanded(
          child: Text('Active Category', style: TextStyle(color: Colors.white)),
        ),
        Switch(
          value: _isActive,
