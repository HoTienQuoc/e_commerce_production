...
import 'package:frontend_admin/features/category/data/models/category_model.dart';
import 'package:frontend_admin/features/category/domain/entities/category_entity.dart';
import 'package:frontend_admin/features/category/domain/entities/paginated_response.dart';
  ...
  getCategories({int page = 1, int pageSize = 20}) async {
    if (await networkInfo.isConnected) {
      try {
        ...
        final processedCategories = _addParentNames(
          paginatedResponse.results.cast<CategoryModel>(),
        );

        return Right(
          PaginatedResponseEntity<CategoryEntity>(
        ...
        return Left(
          ServerFailure(message: "Failed to get categories: ${e.toString()}"),
        );
      }
    } else {
